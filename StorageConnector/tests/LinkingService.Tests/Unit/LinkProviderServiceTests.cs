...
public sealed class LinkProviderServiceTests
{
  ...
  public async Task StartAsync_PersistsStateAndBuildsAuthorizeUrl()
  {
    ...
    var service = new LinkProviderService([oauth], tokenStore, stateStore);
  ...
  {
    var oauth = new RecordingOAuthClient(ProviderType.Google);
    var service = new LinkProviderService([oauth], new FakeTokenStore(), new FakeStateStore());
  ...
  public async Task ConnectCallbackAsync_StoresTokens()
  {
    var oauth = new RecordingOAuthClient(ProviderType.Google)
    {
      ExchangeResult = new TokenSet("access-1", "refresh-1", DateTimeOffset.UtcNow.AddHours(1), ["scope-a", "scope-b"])
    };
