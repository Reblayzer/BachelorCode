...
namespace LinkingService.Tests.Controllers;
...
public sealed class ConnectControllerIntegrationTests : IClassFixture<WebApplicationFactory<LinkingProgram>>
{
      ...
      builder.ConfigureServices(services =>
          {
            ...
            services.AddSingleton<IStateStore>(_ => new LinkingService.Infrastructure.Stores.CacheStateStore(new MemoryCache(new MemoryCacheOptions())));
  ...
  public async Task CallbackEndpoint_ExchangesCode_And_StoresTokens()
  {
    ...
    var stateStore = new LinkingService.Infrastructure.Stores.CacheStateStore(new MemoryCache(new MemoryCacheOptions()));
  ...
  {
    private readonly System.Collections.Concurrent.ConcurrentDictionary<(Guid, ProviderType), ProviderAccount> _store = new();
