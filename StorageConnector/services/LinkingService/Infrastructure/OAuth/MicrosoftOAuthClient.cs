...
namespace LinkingService.Infrastructure.OAuth;
...
public sealed class MicrosoftOAuthClient : IOAuthClient
{
    ...
    public async Task RevokeAsync(string refreshToken)
    {
        ...
        catch (Exception)
        {
            ...
            try { Console.WriteLine("Microsoft RevokeAsync: revoke request failed or not supported."); }
            catch
            {
                // ignored
            }
    ...
    private async Task<T> ReadJsonAsync<T>(HttpResponseMessage response)
    {
        ...
        {
            try { Console.WriteLine($"Microsoft token endpoint returned {(int)response.StatusCode}: {content}"); }
            catch
            {
                // ignored
            }
        ...
        if (result is null)
        {
            try { Console.WriteLine("Microsoft: failed to parse response JSON."); }
            catch
            {
                // ignored
            }
