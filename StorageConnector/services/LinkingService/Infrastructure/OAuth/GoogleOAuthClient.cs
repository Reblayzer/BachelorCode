...
public sealed class GoogleOAuthClient : IOAuthClient
{
    ...
    public async Task RevokeAsync(string refreshToken)
    {
        ...
        catch (Exception)
        {
            ...
            try { Console.WriteLine("Google RevokeAsync: revoke request failed or token already revoked."); }
            catch
            {
                // ignored
            }
    ...
    private async Task<T> ReadJsonAsync<T>(HttpResponseMessage response)
    {
        ...
        {
            try { Console.WriteLine($"Google token endpoint returned {(int)response.StatusCode}: {content}"); }
            catch
            {
                // ignored
            }
        ...
        if (result is null)
        {
            try { Console.WriteLine("Google: failed to parse response JSON."); }
            catch
            {
                // ignored
            }
